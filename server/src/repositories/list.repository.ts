...
class ListRepository extends BaseRepository<any> {
  constructor() {
    super(prisma.list);
  }
  ...
  async createList(input: { boardId: string; name: string }) {
  ...
  async reorder(listId: string, position: number) {
    return await prisma.list.update({
      where: { id: listId },
      data: {
        position,
  ...
  async findCards({ listId }: { listId: string }) {
